...
### 1. Configuração do Spring Boot no IntelliJ

Projeto importado e reconhecido como projeto Maven no IntelliJ IDEA, rodando com JDK 17. O console mostra a aplicação Spring Boot inicializada com sucesso, o perfil `h2` ativo, e o Tomcat embarcado subindo na porta `8083`.

![Aplicação rodando no IntelliJ](docs/intellij-rodando.png)

### 2. Tela de listagem (`/produtos`)
...
![Tela de listagem de produtos, usuário logado](docs/tela-listagem.png)
...
### 5. Formulário de cadastro — Create (`/produtos/novo`)
...
### 6. Formulário de edição — Update (`/produtos/{id}/editar`)
...
Ao confirmar, o produto é removido do banco pelo ID (`POST /produtos/{id}/excluir`) e some da listagem:
